...
This chapter presents an experiment on the use of multimodal artificial intelligence models to support key stages of the camera-trap workflow in biodiversity monitoring. Based on the study by \cite{alencar2024zero}, the chapter examines the application of CLIP, BLIP, GPT-4o, and Gemini 2.0 flash-exp to three tasks: empty-image filtering, species classification, and behavior identification. The text describes the datasets, experimental protocols, evaluation metrics, and reported results, with emphasis on the comparison between zero-shot and few-shot settings and, for behavior analysis, between single-image and sequence-based inference.
