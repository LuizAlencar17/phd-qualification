...
This chapter presents the first empirical study of the thesis, which evaluates the use of multimodal artificial intelligence models to support key stages of the camera-trap workflow in biodiversity monitoring. The study examines the application of CLIP, BLIP, GPT-4o, and Gemini 2.0 flash-exp to three tasks: empty-image filtering, species classification, and behavior identification. It describes the datasets, experimental protocols, evaluation metrics, and results, with emphasis on the comparison between zero-shot and few-shot settings and, for behavior analysis, between single-image and sequence-based inference. An earlier version of this study was published in \citep{alencar2024zero}; in this thesis, the experiment is integrated into the broader research problem of evaluating when multimodal models can support low-supervision camera-trap workflows.
...
This empirical study investigates the use of four state-of-the-art multimodal models---CLIP, BLIP, GPT-4o, and Gemini 2.0 flash-exp---in three core tasks of the camera-trap workflow: empty-image filtering, species classification, and behavior identification. The evaluation compares zero-shot and few-shot strategies and, in the case of behavior recognition, also examines the effect of single-image versus sequence-based inference. In relation to the earlier empty-image filtering study reported in \citep{alencar2024zero}, this chapter broadens the scope from a single-task setting to a multi-task evaluation, incorporates additional models, expands the data from seasons 1--4 to seasons 1--6 of Snapshot Serengeti, and includes class-wise analyses and sequence-based behavioral assessment.
...
The second task was \textit{species classification}. In this case, the models were restricted to a predefined set of nine species: hyena, zebra, giraffe, buffalo, gazelle, wildebeest, elephant, lion, and bird. These classes were selected because they were among the most frequent species in the training data. Empty-image filtering was intentionally separated from species classification because preliminary tests indicated better accuracy for this two-stage formulation than for a joint multi-class formulation.

The third task was \textit{behavior identification}, formulated as a three-class problem with the categories moving, eating, and resting. These behavior classes were selected because they are frequent in labeled camera-trap datasets, feasible to annotate, and relevant for practical wildlife monitoring. This task was evaluated under two input conditions: single images and image sequences corresponding to the same camera-trap event. The number of images per sequence ranged, on average, from 3 to 5.

\subsection{Dataset Splits Used in the Experiments}

Table~\ref{tab:dataset_sizes} presents the distribution of images per subset and task used in this empirical study.

\begin{table}[htbp]
\centering
\caption{Number of images in each subset used for the three evaluated tasks.}
\label{tab:dataset_sizes}

\vspace{0.5em}
\textbf{(a) Empty-image filtering}\\[0.3em]
\begin{tabular}{lrr}
\hline
Subset & Animal & Empty \\
\hline
...
\vspace{1em}
\textbf{(b) Behavior identification}\\[0.3em]
\begin{tabular}{lrrr}
\hline
Subset & Moving & Resting & Eating \\
\hline
Val & 1033 & 988 & 979 \\
Test & 3328 & 3309 & 3363 \\
Train & 6586 & 6732 & 6682 \\
\hline
\end{tabular}

\vspace{1em}
\textbf{(c) Species classification}\\[0.3em]
\begin{tabular}{lrrrrrrrrr}
\hline
Subset & Gazelle & Hyena & Elephant & Lion & Bird & Buffalo & Zebra & Wildebeest & Giraffe \\
\hline
Val & 350 & 347 & 341 & 334 & 333 & 329 & 323 & 322 & 321 \\
Test & 1136 & 1130 & 1127 & 1125 & 1127 & 1136 & 1111 & 1122 & 1103 \\
Train & 2257 & 2248 & 2245 & 2220 & 2161 & 2248 & 2245 & 2209 & 2203 \\
\hline
\end{tabular}
\end{table}
...
Four multimodal models were evaluated: Gemini, GPT, BLIP, and CLIP.

Gemini was included as a multimodal model capable of processing text and images, but its evaluation was restricted to zero-shot learning because it is closed-source and local fine-tuning on the experimental dataset was not feasible. The same limitation applied to GPT-4o. Although GPT supports in-context learning through prompt conditioning, incorporating examples would substantially increase token usage, inference cost, and scalability limitations. For this reason, GPT was also evaluated only in zero-shot mode.
...
In the few-shot setting, models received a small subset of labeled examples before evaluation. CLIP was lightly fine-tuned with a few examples per class. BLIP used sample prompts and corresponding labels for in-context adaptation and was also locally adapted under the same few-shot setup used for CLIP. The comparison between zero-shot and few-shot learning is therefore not fully symmetrical across all models, because only the open models, CLIP and BLIP, allowed local adaptation.
...
The confidence intervals were generally non-overlapping in the main comparisons, which supports the conclusion that the observed performance gaps are unlikely to be attributable only to random variation in the test set.

\subsection{Empty-image Filtering}

In the empty-image filtering task, BLIP in zero-shot mode obtained the lowest performance, with accuracy of 0.0761, precision of 0.0137, recall of 0.0035, and F1-score of 0.0055. After few-shot adaptation, BLIP-FewShot improved substantially, reaching 0.9100 in all four metrics and achieving the best result for this task (see Figure ~\ref{fig:experiment_1-example_animal}).

CLIP performed better than BLIP in zero-shot mode, with accuracy of 0.7987 and F1-score of 0.7959. However, its few-shot version did not improve upon this result; CLIP-FewShot obtained accuracy of 0.7588 and F1-score of 0.7587. This decrease suggests that CLIP may have been negatively affected by the limited number of available training samples.
...
Table~\ref{tab:filter_recall} presents the class-wise recall for the two categories involved in empty-image filtering.
...
\begin{figure*}[!ht]
\centering
\includegraphics[width=6in]{images/experiment_1-example_animal.png}
\caption{Examples of predictions for the filtering empty images task. In these cases, only one model produced the correct prediction, while the others misclassified the image: (a) Only CLIP-FewShot correctly identified the non-empty class; (b) Only BLIP-FewShot correctly identified the image as empty; (c) Only Gemini correctly assigned the non-empty class (the bounding box in the image is just to help see where the animal is, due to the conditions of some images).}
\label{fig:experiment_1-example_animal}
\end{figure*}

\subsection{Species Classification}

In species classification, BLIP again performed very poorly in zero-shot mode, with accuracy of 0.0006, recall of 0.0005, and F1-score of 0.0011. In contrast, BLIP-FewShot reached accuracy of 0.7524 and F1-score of 0.7556. The article therefore identifies a strong dependence of BLIP on few-shot adaptation for this task (see Figure ~\ref{fig:experiment_1-example_species}).
...
The class-wise analysis indicates that giraffe, hyena, and elephant were among the easiest species for the evaluated models. BLIP-FewShot and Gemini both reached recall above 0.88 for giraffe, while GPT reached 0.9442 for hyena. The bird class showed strong contrast among models, with GPT reaching 0.9442 recall and CLIP remaining much lower.

\begin{figure*}[!ht]
\centering
\includegraphics[width=6in]{images/experiment_1-example_species.png}
\caption{Examples of correctly species identification: (a) Only CLIP-FewShot correctly identified a gazelle; (b) Only GPT correctly identified a gazelle; (c) Only BLIP-FewShot correctly identified a lion (the bounding box in the image is just to help see where the animal is, due to the conditions of some images).}
\label{fig:experiment_1-example_species}
\end{figure*}
\subsection{Behavior Identification}

Behavior identification was evaluated both with single images and with image sequences (see Figure ~\ref{fig:experiment_1-example_seq}). In zero-shot mode with single images, BLIP again presented the lowest performance, with accuracy of 0.0011 and F1-score of 0.0004. Using sequences without few-shot adaptation yielded only a limited gain for BLIP-Seq, which reached accuracy of 0.0879 and F1-score of 0.0361.

With few-shot learning, however, BLIP improved substantially. BLIP-FewShot obtained accuracy of 0.7014 and F1-score of 0.7011. When image sequences were added, BLIP-FewShot-Seq achieved the best performance for this task, with accuracy of 0.7557 and F1-score of 0.7567. This result suggests that temporal context can support behavior recognition when combined with few-shot adaptation.
...
The class-wise recall values indicate that the resting class was the most difficult behavior for most models. GPT achieved high recall for moving (0.9026) but much lower recall for resting (0.0731). Gemini and Gemini-Seq showed strong recall for eating and moving, but also low recall for resting. BLIP-FewShot-Seq was the configuration with the most balanced recall across the three behaviors.

\begin{figure*}[!ht]
\centering
\includegraphics[width=3in]{images/experiment_1-example_seq.png}
\caption{Examples of the behavior identification task where only one model produced the correct prediction while all others were incorrect: (a) model BLIP-FewShot-Seq and class resting; (b) model CLIPFewShot-Seq and class moving; (c) model GPT-Seq and class moving; (d) model Gemini-Seq and class moving; (e) model CLIP-Seq and class resting (the bounding box in the image is just to help see where the animal is, due to the conditions of some images).}
\label{fig:experiment_1-example_seq}
\end{figure*}

\section{Discussion}

The results indicate that model behavior varied considerably across tasks, adaptation regimes, and input modalities. The most consistent pattern was the substantial improvement of BLIP under few-shot learning. This was observed in all three tasks, and in behavior identification the best overall result was obtained when few-shot learning was combined with sequence-based input. These findings indicate that BLIP benefits strongly from minimal task-specific supervision and from additional temporal context when behavior needs to be inferred.

A second important observation is that the closed models, although restricted to zero-shot evaluation, achieved competitive results in specific tasks. GPT was the best zero-shot model for empty-image filtering, and Gemini obtained the highest zero-shot accuracy in species classification. In behavior identification, Gemini also benefited from sequence-based input, whereas GPT did not. These results suggest that the usefulness of temporal context depends on the model architecture and is not uniformly beneficial.

The behavior of CLIP was less stable across tasks. In empty-image filtering and species classification, few-shot adaptation led to lower performance than the corresponding zero-shot configuration. This result may indicate overfitting or ineffective adaptation under limited training data. In behavior identification, however, few-shot learning produced gains for CLIP, especially when sequences were used.
...
The results also raise practical considerations for deployment. Although zero-shot and few-shot strategies reduce the need for large-scale retraining compared with conventional CNN pipelines, the evaluated models still impose non-trivial computational costs. BLIP and CLIP require GPU inference, while Gemini and GPT involve API usage costs. For continuous monitoring scenarios, practical systems may therefore require tiered pipelines, lightweight pre-filtering stages, or compression and distillation strategies to reduce operational cost.

Three constraints are particularly relevant for interpreting these results. First, the evaluation of zero-shot and few-shot learning was not symmetrical across models, because only CLIP and BLIP allowed local adaptation. Second, the experiments were conducted on smaller subsets of Snapshot Serengeti due to hardware constraints, even though the source dataset is much larger. Third, real-world deployment would require further consideration of inference cost and system design trade-offs. In addition, the relatively high performance of GPT in the filtering task should be interpreted cautiously, because possible prior exposure to Snapshot Serengeti cannot be ruled out. The dataset was released in 2015, and no earlier GPT snapshot was available for testing.
These findings also reveal an important limitation of using individual multimodal models as isolated inference mechanisms. Although some models achieved competitive results in specific tasks, no single model or configuration was consistently superior across empty-image filtering, species classification, and behavior identification. In species classification, in particular, performance varied across model architectures and species classes, suggesting that visual evidence alone or a single prompt-based inference path may not be sufficient for robust zero-shot recognition. This observation motivates the next chapter, which focuses on species classification and investigates whether combining complementary visual and textual evidence can reduce some of the variability observed in standalone multimodal approaches.
...
In the context of this thesis, this experiment contributes by showing that multimodal AI models can be applied to multiple stages of the camera-trap workflow within a unified evaluation framework. At the same time, the findings indicate that performance is highly conditional: it depends on the selected model, the task, the availability of limited supervision, the use of temporal information, and the species or behavior class being evaluated. This conditional behavior is especially relevant for species classification, where different models showed distinct strengths and weaknesses across classes. Therefore, rather than treating any individual multimodal model as a universally reliable solution, the next chapter investigates a complementary direction: whether visual and textual evidence can be explicitly combined to improve zero-shot species classification in camera-trap workflows.
