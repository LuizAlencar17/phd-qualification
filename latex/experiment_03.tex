...
\midrule
Visual anchor ($e_v$) & CLIP ViT-L/14 image encoder, on the MegaDetector-selected keyframe/crop & Same visual branch as Chapter~\ref{cap:experiment_02}; keyframe/crop selection reuses the \texttt{KeyframeSelector} logic already implemented in the WildReason reference code. \\
Burst context ($e_{seq}$) & Frame-level CLIP embeddings, pooled by a learned attention layer & Generalizes the \texttt{entropy\_weighted}/\texttt{mean}/\texttt{max} frame-aggregation heuristics used in Chapter~\ref{cap:experiment_01} and in WildReason into a trainable pooling operator. \\
...
Ecological knowledge ($e_{ekb}$) & ModernBERT-base encoder over species/behavior knowledge-base text & Extends the species knowledge base of Chapter~\ref{cap:experiment_02} and the EKB structure prototyped in WildReason with an added behavior knowledge base (BKB, defined in Section~\ref{sec:eog}). \\
Spatiotemporal context ($e_{ctx}$) & Small MLP over cyclically encoded timestamp, day period, and camera-location embedding & Formalizes the metadata prompt fields (\texttt{day\_period}, \texttt{location}, \texttt{timestamp}) already present in WildReason's \texttt{metadata\_only} and \texttt{wildreason\_full} modes as a learned embedding rather than free text. \\
\bottomrule
\end{tabular}
\end{table}
...
For each event $i$ in the training corpus (drawn from the union of the Chapter~\ref{cap:experiment_01} and Chapter~\ref{cap:experiment_02} datasets), the visual anchor embedding $e_v^i$ is bound to each available peripheral modality embedding $e_m^i$, $m \in \{seq, cap, ekb, ctx\}$, through a symmetric InfoNCE contrastive objective, following the same principle used to align image and text in CLIP \citep{radford2021learning} and to align sequence and non-sequence modalities in FunBind \citep{boadu2025funbind}:

$$\mathcal{L}_{v,m} = -\frac{1}{N}\sum_{i=1}^{N} \log \frac{\exp\left(\cos(e_v^i, e_m^i)/\tau\right)}{\sum_{j=1}^{N} \exp\left(\cos(e_v^i, e_m^j)/\tau\right)}$$

where $N$ is the batch size, $\tau$ is a learned temperature, and negative pairs $(i,j)$, $i\neq j$, are drawn from other events in the same batch. The total pretraining loss aggregates the per-modality binding losses over the subset of modalities actually available for each batch:

$$\mathcal{L}_{bind} = \sum_{m \in \mathcal{M}} \lambda_m\, \mathcal{L}_{v,m}$$

with $\lambda_m$ weighting each modality's contribution. Only the projection heads and the temperature are trained; the underlying CLIP and ModernBERT encoders remain frozen, following the same rationale used in Chapter~\ref{cap:experiment_02} for the visual and textual branches and in FunBind for its base encoders. Because binding is always performed against the visual anchor rather than pairwise across all modalities, adding a new peripheral modality requires only one new alignment term, not $\mathcal{O}(|\mathcal{M}|^2)$ pairwise terms.

\subsection{Zero-Shot Inference and the Fusion Gate}
...
\subsection{Relation to the WildReason Reference Implementation}

The WildReason codebase already implements several components required by WildBind: event/CSV parsing, EKB loading, MegaDetector-based keyframe and crop selection, CLIP-based frame aggregation, and a heuristic version of uncertainty-gated fusion (margin and entropy thresholds triggering a fixed context weight). WildBind reuses this engineering substrate directly for data loading, image resolution, and keyframe selection, and reuses the margin/entropy diagnostics as the input features $u$ to the learned gate. What WildBind adds on top of this substrate is: (i) the projection heads and contrastive binding objective, currently absent; (ii) the Ecological Ontology Graph and the associated Behavior Knowledge Base, which extend the existing EKB structure beyond species to behavior and presence; (iii) the trainable gate replacing the fixed heuristic thresholds; and (iv) native support for an arbitrary, possibly incomplete, subset of modalities at inference time, replacing the current four hand-engineered ablation modes with four \textit{operating points} of one model (obtained simply by toggling the availability mask $a$), which remain available as a natural ablation study (Section~\ref{sec:eval_ablations}).
...
WildBind -- visual only & $e_v$ & Lower bound; equivalent to WildReason's \texttt{baseline} mode. \\
WildBind -- visual + burst & $e_v, e_{seq}$ & Equivalent to \texttt{multi\_frame\_only}. \\
WildBind -- visual + context & $e_v, e_{ekb}, e_{ctx}$ & Equivalent to \texttt{metadata\_only}. \\
WildBind -- full & $e_v, e_{seq}, e_{cap}, e_{ekb}, e_{ctx}$ & Equivalent to \texttt{wildreason\_full}, now as one model with a learned gate rather than four separate pipelines. \\
