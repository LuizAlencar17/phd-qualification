...
Two fallback conditions are defined for events in which no detection reaches full confidence. If the highest-confidence detection across the event falls below the acceptance threshold $\theta_{det}=0.5$ but above a lower floor of $0.1$, that detection is used as the anchor regardless, and the event is flagged with a binary \textit{low-confidence-anchor} indicator that is appended to the uncertainty vector $u$ consumed by the fusion gate (Section~\ref{sec:fusion_gate}), so that the gate can learn to discount the visual anchor's contribution for these events rather than treating it as equally reliable to a high-confidence crop. If no detection at all reaches the $0.1$ floor, the event is treated as a candidate empty sequence at the detector level: the anchor defaults to the temporally central frame of the burst, encoded without cropping.

This design choice is deliberate: MegaDetector confidence determines only
\textit{which} region of \textit{which} frame becomes the visual anchor,
and how much the gate should trust it, not \textit{whether} the event is
...
Before formalizing the fusion mechanism, it is useful to state plainly what happens, and what is learned, when WildBind processes one camera-trap event. No component of WildBind is trained to directly output a species or behavior label. Instead, every prediction -- presence, species, and behavior -- is obtained through the same two-step operation, repeated once per level of the Ecological Ontology Graph (Section~\ref{sec:eog}): (i) a \textit{retrieval} step, in which each available modality embedding is compared, via cosine similarity, against the embeddings of the current candidate EOG nodes; and (ii) a \textit{fusion} step, in which a small trained network -- the gate $g_\theta$, a two-layer MLP -- combines the resulting per-modality similarity scores into a single fused score per candidate node. The predicted label at each level is simply the candidate node with the highest fused score. The only parameters exposed to labeled data at inference time are therefore those of $g_\theta$; the projection heads that produce the embeddings are frozen after contrastive pretraining (Section~\ref{sec:contrastive_binding}), so adding a previously unseen species or behavior only requires adding a textual description to the corresponding knowledge base (Section~\ref{sec:eog}), not retraining a classifier.
...
scored against the two Level-0 EOG nodes, \textit{empty} and
\textit{animal}. The gate combines these per-modality scores into a fused
score $f_c$ for each node, and $\hat{y}_0=\arg\max_c f_c$ is taken as the
presence prediction. If $\hat{y}_0=\textit{empty}$, inference stops and no
species or behavior label is produced for the event.
\item \textbf{Level 1 -- species.} If $\hat{y}_0=\textit{animal}$, the
candidate set is restricted to the species nodes $S(\textit{animal})$
