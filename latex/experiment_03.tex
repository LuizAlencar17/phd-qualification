...
Consequently, this chapter addresses a core design question left unanswered by previous chapters and current literature: How can multiple heterogeneous evidence sources be integrated into a single learned representation to jointly cover empty-image filtering, species classification, and behavior identification, while preserving a zero-shot, low-supervision regime that is robust to missing modalities at inference time?
...
WildBind is organized as a two-stage architecture that integrates multimodal
event representation with hierarchical retrieval and uncertainty-aware
fusion. Figure~\ref{fig:wildbind_pipeline} provides an overview of the
complete pipeline, from event-level anchor selection and multimodal
encoding to contrastive binding, hierarchical retrieval over the
Ecological Ontology Graph, and the final presence, species, and behavior
predictions.

\begin{figure*}[!ht]
\centering
\includegraphics[width=6in]{images/wildbind_pipeline.png}
\caption{Overview of the WildBind architecture. At the event level, a pretrained detector selects an anchor frame and crop, while four complementary modalities---visual anchor ($e_v$), burst context ($e_{seq}$), generated description ($e_{cap}$), and spatiotemporal context ($e_{ctx}$)---are projected into a shared latent space. The visual anchor is contrastively bound to the peripheral modalities through an InfoNCE objective. At inference time, modality-specific similarity scores are fused by an uncertainty-aware gate and evaluated hierarchically over the Ecological Ontology Graph (EOG): presence, species, and behavior. The retrieval--fusion procedure is repeated at each level with the candidate set restricted by the previous decision; if $\hat{y}_0=\mathrm{Empty}$, inference terminates without producing species or behavior predictions.}
...
Two fallback conditions are defined for events in which no detection reaches full confidence. If the highest-confidence detection across the event falls below the acceptance threshold $\theta_{det}=0.5$ but above a lower floor of $0.1$, that detection is used as the anchor regardless, and the event is flagged with a binary low-confidence-anchor indicator that is appended to the uncertainty vector $u$ consumed by the fusion gate (Section~\ref{sec:fusion_gate}), allowing the gate to learn to discount the visual anchor's contribution for these events, dynamically adjusting its reliance based on the detection confidence. If no detection at all reaches the $0.1$ floor, the event is treated as a candidate empty sequence at the detector level: the anchor defaults to the temporally central frame of the burst, encoded without cropping.

This design choice is deliberate: detector confidence determines only
which region of which frame becomes the visual anchor,
and how much the gate should trust it, not whether the event is
ultimately classified as empty or as containing an animal. That decision
remains the responsibility of the Level-0 retrieval step
(Section~\ref{sec:fusion_gate}), which combines the fused similarity score
$f_c$ against the two Level-0 EOG nodes using every available modality --
not only the visual anchor -- so that a missed detection
lowers the quality and trustworthiness of the visual evidence rather than
automatically determining the final presence label.
Section~\ref{sec:limitations_discussion} returns to the consequences of
this dependency for events in which the detector fails to detect a present
animal in any frame.
...
\begin{itemize}
\item \textbf{Level 0 -- Presence.} Two nodes, empty and animal, each represented by a short textual description analogous to the filtering prompts used in Chapter~\ref{cap:experiment_01}.
\item \textbf{Level 1 -- Species.} One node per candidate species, represented by the visually oriented summaries already curated for the species knowledge base in Chapter~\ref{cap:experiment_02}, conditioned on the parent animal node.
...
\caption{The three-level Ecological Ontology Graph, illustrated with an example path. Level 0 separates presence (empty vs. animal); Level 1 conditions the species candidate set on the parent animal node; Level 2 conditions the behavior candidate set on the parent species node. The highlighted path (animal $\rightarrow$ lion $\rightarrow$ eating) illustrates one hierarchical prediction $(\hat{y}_0, \hat{y}_1, \hat{y}_2)$; empty is a leaf node, since $S(\text{empty})=\emptyset$.}
...
\item \textbf{Level 0 -- presence.} Every available modality embedding is
scored against the two Level-0 EOG nodes, empty and
animal. The gate combines these per-modality scores into a fused
score $f_c$ for each node, and $\hat{y}_0=\arg\max_c f_c$ is taken as the
presence prediction. If $\hat{y}_0=empty$, inference stops and no
species or behavior label is produced for the event.
\item \textbf{Level 1 -- species.} If $\hat{y}_0=animal$, the
candidate set is restricted to the species nodes $S(animal)$
conditioned on the Level-0 decision, and the same retrieval-and-fusion
procedure yields $\hat{y}_1$.
\item \textbf{Level 2 -- behavior.} The candidate set is further restricted
to the behavior nodes $B(\hat{y}_1)$ conditioned on the predicted species,
and the same procedure yields the final label $\hat{y}_2$.
\end{enumerate}
