...
\caption{Overview of the WildBind architecture. At the event level, MegaDetector v5 selects an anchor frame and crop, while five complementary modalities---visual anchor ($e_v$), burst context ($e_{seq}$), generated description ($e_{cap}$), ecological knowledge ($e_{ekb}$), and spatiotemporal context ($e_{ctx}$)---are projected into a shared latent space. The visual anchor is contrastively bound to the peripheral modalities through an InfoNCE objective. At inference time, modality-specific similarity scores are fused by an uncertainty-aware gate and evaluated hierarchically over the Ecological Ontology Graph (EOG): presence, species, and behavior. The retrieval--fusion procedure is repeated at each level with the candidate set restricted by the previous decision; if $\hat{y}_0=\mathrm{Empty}$, inference terminates without producing species or behavior predictions.}
...
\begin{enumerate}
\item \textbf{Modality extraction.} The visual anchor is selected and encoded into $e_v$. The
peripheral embeddings $e_{seq}$, $e_{cap}$, $e_{ekb}$, and $e_{ctx}$ are
computed for every modality available for this event; any modality that
cannot be computed for a given event (e.g., a missing timestamp) is marked
unavailable in the modality-availability mask $a$ rather than imputed with
a default value.
