...
\caption{Overview of the WildBind architecture. At the event level, MegaDetector v5 selects an anchor frame and crop, while four complementary modalities---visual anchor ($e_v$), burst context ($e_{seq}$), generated description ($e_{cap}$), and spatiotemporal context ($e_{ctx}$)---are projected into a shared latent space. The visual anchor is contrastively bound to the peripheral modalities through an InfoNCE objective. At inference time, modality-specific similarity scores are fused by an uncertainty-aware gate and evaluated hierarchically over the Ecological Ontology Graph (EOG): presence, species, and behavior. The retrieval--fusion procedure is repeated at each level with the candidate set restricted by the previous decision; if $\hat{y}_0=\mathrm{Empty}$, inference terminates without producing species or behavior predictions.}
...
\midrule
Visual anchor ($e_v$) & CLIP ViT-L/14 image encoder, on the MegaDetector-selected keyframe/crop & Same visual branch as Chapter~\ref{cap:experiment_02}; anchor-selection criteria and fallback rule detailed below. \\
...
This design choice is deliberate: MegaDetector confidence determines only
...
not only the visual anchor -- so that a missed MegaDetector detection
...
this dependency for events in which MegaDetector fails to detect a present
animal in any frame.
